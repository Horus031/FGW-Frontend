...
export function SharedMajorProvider({ children, initialMajor = defaultMajor }: ProviderProps) {
  const { data: programmeData, isLoading: isProgrammeLoading } = useQuery({
    queryKey: ["programmes"],
    queryFn: getAllProgrammes,
  });

  const [major, setMajor] = useState<MajorState>(() => {
    if (initialMajor?.programme?.id && initialMajor.programme.id !== 0) {
      return initialMajor;
    }
    // fallback to default until programmes arrive
    return defaultMajor;
  });
  ...
  // Only update major *after* programmes load and only if still default
  useEffect(() => {
    if (
      programmeData &&
      programmeData.length > 0 &&
      (!major?.programme?.id || major.programme.id === 0)
    ) {
      const first = programmeData[0];
      // schedule asynchronously to avoid synchronous setState warning
      queueMicrotask(() => {
        setMajor((prev) => ({
          ...prev,
          programme: { index: 0, id: Number(first.id) },
  ...
  const value = useMemo(
    () => ({
      ...
      programmeData,
      termData,
      isLoading: isProgrammeLoading || isTermLoading,
    }),
    [major, programmeData, termData, isProgrammeLoading, isTermLoading]
  );

  return (
    <SharedMajorContext.Provider value={value}>
      {children}
    </SharedMajorContext.Provider>
  );
}
